...
module.exports = function (grunt) {
    ...
    var taskConfig = {
        ...
        clean: {
            all: {
                src: [  '<%= build_dir %>**',
                        ...
                        '<%= webapp %>/src',
                        '<%= webapp %>/index.html',
                        '<%= webapp %>/templates-app.js',
                        '<%= webapp %>/templates-common.js',
                        '<%= webapp %>/bham-version.json'],
            ...
            webappDebug: {
              src : ['<%= webapp %>/assets', '<%= webapp %>/src', '<%= webapp %>/index.html', '<%= webapp %>/templates-app.js', '<%= webapp %>/templates-common.js']
           },

            webappDist: {
                src : ['<%= webapp %>/assets', '<%= webapp %>/index.html', '<%= webapp %>/bham-version.json']
            }
        },
        ...
        concat: {
            ...
            compile_js: {
                ...
                src: [
                    '<%= vendor_files.js %>',
                    '<%= build_debug_dir %>/src/main/webapp/source/**/*.js',
                    '<%= html2js.app.dest %>',
                    '<%= html2js.common.dest %>'
                ],
    ...
    grunt.registerMultiTask('index', 'Process index.html template', function () {
        ...
        grunt.file.copy('src/main/webapp/source/index.html', this.data.dir + '/index.html', {
...
// Alias task for karma:watch. Run this task after grunt build
    ...
    grunt.registerTask('build', function (target) {
        ...
        if (target === targetEnum.all) {
            ...
            taskList = [
                'clean:all', 'bower:install', 'html2js', 'jshint-all', 'coffeelint', 'coffee', 'recess:build',
            ...
            if (target === targetEnum.debug ) {
                taskList = taskList.concat(['copy:debug']);
            }

            if (target === targetEnum.dist || target === targetEnum.ci ) {
                taskList = taskList.concat(['compile', 'save-version', 'compress', 'copy:dist']);
            }
